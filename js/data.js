...
function exportCSV(){WHT.haptic('medium');var r=WHT.getUserRecords();var csv=[['\u65e5\u671f','\u5f00\u59cb\u65f6\u95f4','\u7ed3\u675f\u65f6\u95f4','\u5de5\u65f6','\u8282\u5047\u65e5','\u5907\u6ce8']].concat(r.map(function(x){return[x.date,x.startTime,x.endTime,x.hours,x.isHoliday?'\u662f':'\u5426',x.note||''].map(csvEscape)})).map(function(x){return x.join(',')}).join('\n');var csvContent='\ufeff'+csv;var blob=new Blob([csvContent],{type:'text/csv;charset=utf-8'});var filename='work-hours-'+WHT.today()+'.csv';shareOrDownload(blob,filename,'text/csv',csvContent)}
